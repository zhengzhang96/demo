function replay_RecordedData_IQ(Data_Matrix_1,frame_axis,range_axis,actual_fps,visualise_fps,subj_name,time,recording_actual_time,total_time,frame_start,frame_stop,frame_stamp_vec,freqRes_STFT)
...
clc
disp('------------------------------------------------');
disp(['Information about recorded data: ' newline]);
disp(['Subject''s Name/ID:  ' subj_name]);
disp('Data type:  Baseband (I/Q)');
disp(['Recording Date (File ID):  ' time(7:8) '/' time(5:6) '/' time(1:4)]);
disp(['Recording Time (File ID):  ' time(10:11) ':' time(12:13) ':' time(14:15)]);
...
disp(['------------------------------------------------' newline]);
disp('Replaying Recorded Data...');


% Initialise figure
fh_baseband = figure('Name','Replaying Recorded Baseband signals','NumberTitle','off','Position', [218 62 1100 700]);
clf(fh_baseband);
...
% set(ax_doppler,'CLim',[-50 5]);
set(ax_doppler,'CLim',[0.99 1]);
colormap(jet)
...
r=1; % Naka–Rushton equation parameter
...
%         clim = get(ax_doppler,'CLim'); %CLim ="color limits".
%         set(ax_doppler,'CLim',clim(2) + [-50 -5]);
        
%         x = 20*log10(abs(conj(STFT))+eps);
%         max(max(x))
        ph_doppler.CData = STFT_new;
%         lower_lim_STFT = mean(STFT_new,'all') + 3*std(STFT_new,[],'all');
        
        
        general_title.String = sprintf('Recorded Data - Elapsed Time = %.2f s',round(frame_axis(i)+dt,2));
