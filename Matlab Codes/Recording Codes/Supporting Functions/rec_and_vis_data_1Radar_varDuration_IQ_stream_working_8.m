function [fh_baseband]=rec_and_vis_data_1Radar_varDuration_IQ_stream_working_8(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
...
%Trajectory
current_traj = 0;
traj = zeros(1,tot_frame_num_history/50);
r=1; %Contrast enhancement parameter
fcut = 0.004;
b_coeffs = fir1(25,fcut,'scale');
breathing_frange = [0.12 1.2];
breathing_time = 5;%in sec
breathing_samples = breathing_time*actual_fps;
...
%     Trajectory
%     subplot(4,4,[3,4])
%     sh_signal = scatter(0,0); hold on
%     tr_signal = plot(NaN,NaN);hold off
%     ylabel('Range (m)'); xlabel('Past Time (s)');
%     tr_signal.YData = range_vector; %x-axis values
%     ylim([range_vector(1) range_vector(end)]);
%     tr_signal.XData = 0:dt_plot:history_duration;%(-history_duration+dt_plot*(4/5):dt_plot:dt_plot/5);    
%     xlim([-history_duration+dt_plot*(4/5) dt_plot/5]);
% %     ph_history.CData = Data_Matrix_1_plot(1:plot_steps:round(history_duration/dt),:);
%     
%      %ph stands for plot handle
%     th_signal = title(''); %th stands for title handle
%     th_signal.String = 'Trajectory';
%     ax_signal = gca; %ax stands for axes handle
%     ax_signal.FontSize = 10;
%     ylim(ax_signal,[0 0.01]);
%     xlim([range_vector(1) range_vector(end)]);
...
%     ax_doppler_right.FontSize = 12;
    ...
    ph_history = imagesc(0,0,0);hold on
    th_history = title('');
...
%     ax_history.FontSize = 12;
    ...
    tr_signal = plot(traj,(-history_duration+dt_plot:dt_plot:0),'k--','Linewidth',2.5);
    loc = plot(NaN,0,'^','MarkerSize',10,'MarkerFaceColor','k');
...
%     yyaxis left
    ...
    th_rawdata = title('');
    th_rawdata.String = 'Respiratory Band Signal';
    ax_rawdata = gca;
    ...
    xlim([frame_vector(end-breathing_samples) frame_vector(end)]);
...
while keep
    ...
    if info_1>history_duration/dt
        ...
        Data_Matrix_1_plot(end,:)=abs(Data_Matrix_1_unfilt(3,:));
    end
        ...
        if visualise_data
            ...
            if info_1>round(history_duration/dt,0)
                ...
                if(walking_speed == 0 && object == 1)
                    ...
                    detrend_data = detrend(Data_Matrix_1_plot(end-breathing_samples:end,:));
                    Data_Matrix_1_abs = detrend(abs(Data_Matrix_1_plot(end-breathing_samples:end,:)));
...
%                     temp = modes(loc,:);
    ...
    %               [fftmax,energy,FFT_signal,fftFreqs]
                    [fftmax,~,FFT_signal,FFTfreq] = fftmax_with_boundarise(real(detrend_data(:,range_index)),fps,breathing_frange);
...
%                     th_respiratory_fft.String = {'Respiratory data';str};
                ...
                traj = circshift(traj,-1);
                traj(end) = current_traj;
                
                tr_signal.XData = traj;
                loc.XData = current_traj;
                ph_rawdata.YData = real(detrend_data(end-breathing_samples:end,range_index));
                ph_rawdata.XData = frame_vector(end-breathing_samples:end);
                xlim(ax_rawdata, [frame_vector(end-breathing_samples) frame_vector(end)]);
...
%                 ph_doppler.CData = 20*log10(abs(conj(STFT))+eps);
                ...
                if sign(walking_speed)==1
                    ...
                    general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                            'Direction: \bf{\color{blue}APPROACHING}\rm, Walking velocity: ' sprintf(' \\bf%.3f\\rm',walking_speed) ' \bfm/s\rm' newline ...
                                            'Number of Steps: ' sprintf('\\bf%.0f\\rm',forward_step_counter) ' , Step Length: ' sprintf('\\bf%.3f\\rm',step_length) ' \bfm\rm , Cadence: ' sprintf('\\bf%.3f\\rm',cadence) ' \bfsteps/min\rm'];
                elseif sign(walking_speed)==-1
                    general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                            'Direction: \bf{\color{darkgreen}RECEDING  }\rm, Walking velocity: ' sprintf('\\bf%.3f\\rm',walking_speed) ' \bfm/s\rm' newline ...
                                            'Number of Steps: ' sprintf('\\bf%.0f\\rm',backward_step_counter) ' , Step Length: ' sprintf('\\bf%.3f\\rm',step_length) '\bf m\rm , Cadence: ' sprintf('\\bf%.3f\\rm',cadence) ' \bfsteps/min\rm'];
                else
                    if object == 1

                        general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                'Direction: \bf{\color{red}BREATHING }\rm, respiration Rate: ' sprintf(' \\bf%.0f\\rm',round(fftmax*60)) '\bf m/s\rm' newline ...
                                                'Number of Steps: \bf0\rm, Step Length: ' sprintf('\\bf%.3f\\rm',step_length) ' \bfm\rm , Cadence: ' sprintf('\\bf%.3f\\rm',cadence) ' \bfsteps/min\rm'];
                    else
                        general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                'Direction: \bf{\color{black}None }\rm, Walking velocity: ' sprintf(' \\bf%.3f\\rm',walking_speed) '\bf m/s\rm' newline ...
                                                'Number of Steps: \bf0\rm, Step Length: ' sprintf('\\bf%.3f\\rm',step_length) ' \bfm\rm , Cadence: ' sprintf('\\bf%.3f\\rm',cadence) ' \bfsteps/min\rm'];
                    end 
                end
                drawnow;
