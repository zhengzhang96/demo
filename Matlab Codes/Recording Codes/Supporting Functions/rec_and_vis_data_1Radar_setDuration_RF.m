...
if visualise_data
    fh_RF = figure('Name','Recording RF signals','NumberTitle','off','Position',[218 62 1100 700]);
...
if ~calibration %Only when not in calibration process.
    cal_filename = '../../Matlab Codes/Recording Codes/Recordings MAT files/Radar_Data_Demo_RF_Calibration.mat'; % Same filename always
...
while (total_time<total_duration)
    ...
    if ~calibration
        ...
        Data_Matrix_1(info_1,:)=data_1'; %Attach each vector (iq_vec' is row vector) in next row.
    end
