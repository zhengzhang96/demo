function [fh_RF,Data_Matrix_1,bin_length,range_vector,total_time,info_1,frame_stamp_vec,queue,time_toc,recording_actual_time,frame_area_tot,range_vector_length]=rec_and_vis_data_1Radar_varDuration_RF(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
...
total_duration = 5*60; % Convert mins to seconds
total_duration = total_duration + history_duration; % Add history duration to the total time (history_duration seconds for calibration)
...
if visualise_data
    fh_RF = figure('Name','Recording RF signals','NumberTitle','off','Position',[218 62 1100 700],'KeyPressFcn',@key_pressed_fcn);
...
if ~calibration %Only when not in calibration process.
    cal_filename = '../../Matlab Codes/Recording Codes/Recordings MAT files/Radar_Data_RF_Calibration.mat'; % Same filename always
...
disp([newline 'After initialization, press any key to stop recording...'])

while (total_time<total_duration) && keep %% && ishandle(ButtonHandle)
    ...
    Data_Matrix_1_unfilt(info_1,:) = data_1'; %Attach each vector (data' is row vector) in next row.

    % High Pass filtering stage - 2nd Order Butterworth
    if info_1>=3 % Order+1
        Data_Matrix_1(info_1,:) = b_hp(1)*Data_Matrix_1_unfilt(info_1,:) + b_hp(2)*Data_Matrix_1_unfilt(info_1-1,:) + b_hp(3)*Data_Matrix_1_unfilt(info_1-2,:)...
            -a_hp(2)*Data_Matrix_1(info_1-1,:) - a_hp(3)*Data_Matrix_1(info_1-2,:);
    else
        Data_Matrix_1(info_1,:) = Data_Matrix_1_unfilt(info_1,:);
    end
    
    % Convert data from IQ to RF
    RF_data_complex = (Data_Matrix_1(info_1,:))'.*complex_fact;
    ...
    Data_Matrix_1_IQ(info_1,:) = conj(IQ_data);
    ...
    if info_1>history_duration/dt
        Data_Matrix_1_plot(info_1,:)=Data_Matrix_1(info_1,:);
    end
    ...
    if ~mod(info_1,visualise_steps) && info_1>=win_upper_ind(1) && info_1<=win_upper_ind(end) % Plot at the defined visualisation frame rate
        if visualise_data
            ...
            if info_1>round(history_duration/dt,0)
                ...
                ph_signal_RF.YData = Data_Matrix_1(info_1,:)';
                ph_signal_IQ.YData = abs(Data_Matrix_1_IQ(info_1,:))';
                peak_val = max(Data_Matrix_1(info_1,:));
                ...
                if total_time<=history_duration
                    ph_history.CData = abs(Data_Matrix_1_plot(1:plot_steps:info_1,:));
                elseif total_time>history_duration
                    ph_history.CData = abs(Data_Matrix_1_plot((info_1-floor(history_duration/dt)):plot_steps:info_1-plot_steps,:));
                end
...
% Truncate the output data matrices and frame_stamp vector to the actual
% recording time.
Data_Matrix_1=Data_Matrix_1(1:info_1,:);
frame_stamp_vec = frame_stamp_vec(1:info_1);
time_toc = time_toc(1:info_1);
queue = queue(1:info_1);
