% Recording IQ data for 1 Radar for a set duration of recording.
...
[~,Data_Matrix_1,bin_length,range_vector,total_time,info_1,frame_stamp_vec,queue,time_toc,recording_actual_time,frame_area_tot,range_vector_length]=rec_and_vis_data_1Radar_setDuration_IQ(xep_1,frame_start,frame_stop,actual_fps,fps,total_duration,visualise_data,visualise_fps,calibration,freqRes_STFT);
...
% figure;
% % Visualise which frames are dropped from Radar
% % 1 - Not dropped frame, 0 - Dropped frame.
% frame_stamp_Radar1 = (frame_stamp_vec==0);
k = find(frame_stamp_vec==0);
% stem(frame_axis,frame_stamp_Radar1,'LineStyle','none');
% title('Visualising dropped frames'); xlabel('Frame index');
% ylim([-0.1 1.1]);
