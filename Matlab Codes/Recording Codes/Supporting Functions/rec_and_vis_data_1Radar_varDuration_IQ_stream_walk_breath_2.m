function [fh_baseband]=rec_and_vis_data_1Radar_varDuration_IQ_stream_walk_breath_2(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
...
f_carrier = 7.29e9; %center frequency of carrier sinusoid.
...
fcut = 1/8;
b_coeffs = fir1(25,fcut,'scale');
breathing_frange = [0.12 1.2];
% Note: To find screen size use:
% set(0,'units','pixels')  
% %Obtains this pixel information
% Pix_SS = get(0,'screensize')
plot_end = datetime;
plot_start = plot_end - seconds(history_duration);
frame_vector = linspace(plot_start,plot_end,tot_frame_num_history);
breathing_counter = 0;
nothing_counter = 0;
motion_counter = 0;
current_state = 0;
...
if visualise_data
    ...
    ax_signal.FontSize = 10;
    ylim(ax_signal,[0 0.01]);
...
%      % Plotting the raw breathing data
%     subplot(4,4,[13,14])
% 
% %     yyaxis left
%     ph_rawdata = plot(frame_vector(1), range_vector(1),'linewidth',2);
%     th_rawdata = title('');
%     th_rawdata.String = 'Raw radar data and confidence level';
%     ax_rawdata = gca;
%     set(ax_rawdata,'YDir','normal')
%     % ylim(ax_rawdata,[0 5]);
%     xlabel('Time [s]'); ylabel('data Amp');
%     ph_rawdata.XData = frame_vector; %x-axis values
%     ph_rawdata.YData = zeros(1,tot_frame_num_history)';
%     xlim([frame_vector(1) frame_vector(end)]);
% 
% %     yyaxis right
% %     ph_confi = plot(frame_vector_con(1),range_vector(1),'linewidth',2);
% %     ax_confi = gca;
% %     set(ax_confi,'YDir','normal')
% %     ylim(ax_rawdata,[0 1]);
% %     xlabel('Time [s]'); ylabel('confidence level');
% %     ph_confi.XData = frame_vector_con; %x-axis values
% %     ph_confi.YData = zeros(1,tot_frame_num_plot)';
% %     xlim([frame_vector_con(1) frame_vector_con(end)]);
% 
% %     % Plotting the Respiratory data
% %     subplot(4,4,[11,12])
% %     ph_respiratory = plot(frame_vector(1), range_vector(1),'linewidth',2);
% %     th_respiratory = title('');
% %     th_respiratory.String = 'Respiratory data';
% %     ax_respiratory = gca;
% %     xlabel('Time [s]'); ylabel('Magnitude (AU)');
% %     xlim(ax_respiratory, [frame_vector(1) frame_vector(end)]);
% %     ph_respiratory.XData = frame_vector; %x-axis values
% %     ph_respiratory.YData = zeros(1,tot_frame_num_history)';
% 
% 
%     % Plotting the Respiratory FFT
%     subplot(4,4,[15,16])
%     ph_respiratory_fft = plot(NaN, NaN,'linewidth',2);
%     th_respiratory_fft = title('');
%     th_respiratory_fft.String = 'Respiratory FFT';
%     ax_respiratory_fft = gca;
%     xlabel('Frequency [s]'); ylabel('Magnitude (AU)');
%     xlim(ax_respiratory_fft, [breathing_frange(1),breathing_frange(2)]);
...
% f_c = 0.2;
[b1,a1] = butter(2,f_c/(actual_fps/2),'high');

[b2,a2] = butter(4,breathing_frange/(actual_fps/2),'bandpass');
...
while keep
    ...
    if info_1>history_duration/dt
        ...
        Data_Matrix_1_plot(end,:)=iq_vec_1.';
    end
    
%     if info_1>=win_lower_ind_start
%         Data_Matrix_1_Status = circshift(Data_Matrix_1_Status,-1,1);
%         Data_Matrix_1_Status(end,:) = iq_vec_1.';
%     end
%     if abs(sum(iq_vec_1)) - abs(sum(calibration_vector)) > 
    ...
    if info_1>=3 % Order+1 to avaoid calculations involving NaN's
        Data_Matrix_1(3,:) = b1(1)*Data_Matrix_1_unfilt(3,:) + b1(2)*Data_Matrix_1_unfilt(2,:) + b1(3)*Data_Matrix_1_unfilt(1,:)...
            -a1(2)*Data_Matrix_1(2,:) - a1(3)*Data_Matrix_1(1,:);
    else
    ...
    if info_1>=win_lower_ind_start
        ...
        Data_Matrix_1_STFT(end,:) = Data_Matrix_1_unfilt(3,:);
    end
    ...
    if ~mod(info_1,visualise_steps) && info_1>=win_upper_ind_start % Plot at the defined visualisation frame rate
        ...
        if visualise_data
            ...
            if info_1>round(history_duration/dt,0)
                variance = var(sum(abs(Data_Matrix_1_plot(end-500:end,:)),2));
...
%                     if current_state~=2
                        current_state = 0;
%                     end
                elseif(variance>1000e-7)
                    current_state = 1;
                    breathing_count = 0;
                else
                    breathing_count = breathing_count + 1;
                    if breathing_count == 20 
                        current_state = 2;
                        breathing_count = 0;
                    end
                end
                ...
                if current_state == 1
                    response = freqz(b1,a1,size(STFT_new,1));
                    STFT_new = STFT_new.*repmat(abs(response),[1,size(STFT_new,2)]);
                elseif current_state == 2
                    response = freqz(b2,a2,size(STFT_new,1));
                    STFT_new = STFT_new.*repmat(abs(response),[1,size(STFT_new,2)]);
                end
                ...
                if reset_steps
                    smooth_envelope_OverTime = zeros(1,3);
                    past_vals = zeros(1,2);
                    past_vals_locs = zeros(1,2);
                    possible_peak=0;
                    local_peak_counter = 0;
                    forward_step_counter = 0;
                    backward_step_counter = 0;
                    min_dist = 200;
                    peak_loc = zeros(1,3); % Store two peak locations to compare them.
                    step_loc = zeros(2,3); % Each row stores information of one step.
                    step_length=0;
                    cadence = 0;
                end
                ...
                if Torso_val>0.7
                    ...
                    if (sign(walking_speed)+sign(Torso_speed))==0 % Opposite to each other
                        ...
                        if abs(Torso_speed)>0.1 %threshold for velocity
                            ...
                            % Smooth the envelopes over Range
                            upper_envelope = movmean(upper_envelope,3);
                            lower_envelope = movmean(lower_envelope,3);

                            if sign(walking_speed)==1

                                % Smoothing envelove peak values over time.
                                smooth_envelope_OverTime = circshift(smooth_envelope_OverTime,-1);
                                smooth_envelope_OverTime(end) = mean(maxk(upper_envelope,3));
                                non_zero_elements = nnz(smooth_envelope_OverTime); % Compute the number of non-zero elements.

                                past_vals = circshift(past_vals,-1);
                                past_vals_locs = circshift(past_vals_locs,-1);
                                past_vals(2) = sum(smooth_envelope_OverTime)/non_zero_elements;
                                [~,range_loc_ind] = maxk(upper_envelope,3);
                                past_vals_locs(2) = round(mean(range_loc_ind));
                                if ~possible_peak && (past_vals(2)>=past_vals(1))
                                    possible_peak = 1;
                                end
                                if possible_peak && (past_vals(2)<past_vals(1))
                                    local_peak_counter = local_peak_counter+1;
                                    if ((info_1 - peak_loc(2))<=min_dist) && (any(peak_loc))
                                        if past_vals(1)>peak_loc(3)
                                            peak_loc = [past_vals_locs(1),info_1,past_vals(1)];
                                        end
                                    elseif ((info_1 - peak_loc(2))>min_dist) && (any(peak_loc))
                                        step_loc = circshift(step_loc,-1,1);
                                        step_loc(2,:) = peak_loc;
                                        % Compare steps if we have at least two steps (starts from onset of 3rd step)
                                        if any(step_loc(1,:))
                                            step_length_new = abs(step_loc(2,1)-step_loc(1,1))*bin_length;
                                            cadence_new = 60/(abs(step_loc(2,2)-step_loc(1,2))*dt);
                                            step_length = step_length + (1/(forward_step_counter-1))*(step_length_new - step_length);
                                            cadence = cadence + (1/(forward_step_counter-1))*(cadence_new - cadence);
                                        end
                            ...
                            elseif sign(walking_speed)==-1
                                ...
                                % Smoothing envelove peak values over time.
                                smooth_envelope_OverTime = circshift(smooth_envelope_OverTime,-1);
                                smooth_envelope_OverTime(end) = mean(maxk(abs(lower_envelope),3));
                                non_zero_elements = nnz(smooth_envelope_OverTime); % Compute the number of non-zero elements.

                                past_vals = circshift(past_vals,-1);
                                past_vals_locs = circshift(past_vals_locs,-1);
                                past_vals(2) = sum(smooth_envelope_OverTime)/non_zero_elements; 
                                [~,range_loc_ind] = maxk(abs(lower_envelope),3);
                                past_vals_locs(2) = round(mean(range_loc_ind));
                                if ~possible_peak && (past_vals(2)>=past_vals(1))
                                    possible_peak = 1;
                                end
                                if possible_peak && (past_vals(2)<past_vals(1))
                                    ...
                                    elseif ((info_1 - peak_loc(2))>min_dist) && (any(peak_loc))
                                        ...
                                        if any(step_loc(1,:))
                                            step_length_new = abs(step_loc(2,1)-step_loc(1,1))*bin_length;
                                            cadence_new = 60/(abs(step_loc(2,2)-step_loc(1,2))*dt);
                                            step_length = step_length + (1/(backward_step_counter-1))*(step_length_new - step_length);
                                            cadence = cadence + (1/(backward_step_counter-1))*(cadence_new - cadence);
                                        end
...
%                         elseif abs(Tosor_freq)>0.2 && abs(Tosor_freq) <1.2
%                             walking_speed=0;
%                             counter_walking=0;
%                             reset_steps = 1;
%                             Raspration_rate = 0.8*Raspration_rate + 0.2*f_STFT(f_ind);
%                             ph_torso_loc.XData=range_loc_torso;
%                             ph_torso_loc.YData=f_STFT(f_ind);    
                        end
                    end
                else
                    counter_walking = 0;
...
%                 if(Torso_val>0.95)&&(walking_speed == 0)
%                     range_index = round(mean(range_indices(f_indices)));
%                     if(range_index-round(0.05/bin_length)<=1)
%                         subject_range = 1:(range_index+round(0.05/bin_length));
%                     elseif(range_index+round(0.05/bin_length)>=length(range_vector))
%                         subject_range = (range_index-round(0.05/bin_length)):length(range_vector);
%                     else
%                         subject_range = (range_index-round(0.05/bin_length)):(range_index+round(0.05/bin_length));
%                     end
%                     detrend_data = detrend(Data_Matrix_1_plot);
%                     Data_Matrix_1_abs = detrend(abs(Data_Matrix_1_plot));
%                     fil_res = filtfilt(b_coeffs,1,double(Data_Matrix_1_abs(:,subject_range)));
%                     abs_bin = sum(abs(fil_res),2);
%                     
%                     abs_bin = filtfilt(b_coeffs,1,double(abs_bin));
% 
...
%                     
%                     
%     %               [fftmax,energy,FFT_signal,fftFreqs]
%                     [fftmax,~,FFT_signal,FFTfreq] = fftmax_with_boundarise(abs_bin,fps,breathing_frange);
%                    
%                     
%                     
%                     plot_end = datetime;
%                     plot_start = datetime - seconds(history_duration);
%                     frame_vector = linspace(plot_start,plot_end,tot_frame_num_history);
% %                     frame_vector_con = linspace(plot_start,plot_end,tot_frame_num_plot);
% 
% %                     ph_respiratory.XData = frame_vector;
% %                     ph_respiratory.YData = normalize(abs_bin,'range');
% %                     xlim(ax_respiratory, [frame_vector(1) frame_vector(end)]);
%                     str = ['Estimated Respiratory is:' num2str(round(fftmax*60)),'/min'];
%                     th_respiratory_fft.String = {'Respiratory data';str};
%                     
%                     ph_respiratory_fft.YData = abs(FFT_signal);
%                     ph_respiratory_fft.XData = FFTfreq;
%                     
%                     ph_rawdata.YData = real(detrend_data(:,range_index));
%                     ph_rawdata.XData = frame_vector;
%                     xlim(ax_rawdata, [frame_vector(1) frame_vector(end)]);
%                     
%                 end
                ...
                peak_val = max([0.01 max(abs(Data_Matrix_1(3,:))) + 0.005]);
...
%                 ph_doppler.CData = 20*log10(abs(conj(STFT))+eps);
                ...
                try
                    if  current_state == 0
                        general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                'Status: \bf{\color{black}None }\rm Variance: ' sprintf(' \\bf%.3f\\rm',variance*1e7) newline];
                    elseif current_state == 2
                        general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                'Status: \bf{\color{red}Breathing }\rm Variance: ' sprintf(' \\bf%.3f\\rm',variance*1e7) ' Location: ' sprintf(' \\bf%.0f\\rm',range_loc_torso)  newline ...
                                                'Raspration rate: ' sprintf(' \\bf%.0f\\rm',abs(f_STFT(f_ind))) '\bf Hz\rm' ];
                    else
                        if sign(walking_speed)==1

                            general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                    'Status: \bf{\color{blue}APPROACHING}\rm Variance' sprintf(' \\bf%.3f\\rm',variance*1e7) ' Location: ' sprintf(' \\bf%.0f\\rm',range_loc_torso) newline ...
                                                    'Walking velocity: ' sprintf(' \\bf%.3f\\rm',walking_speed) '\bfm/s\rm, Number of Steps: ' sprintf('\\bf%.0f\\rm',forward_step_counter) ' , Step Length: ' sprintf('\\bf%.3f\\rm',step_length) ' \bfm\rm , Cadence: ' sprintf('\\bf%.3f\\rm',cadence) ' \bfsteps/min\rm'];
                        elseif sign(walking_speed)==-1
                            general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                    'Status: \bf{\color{darkgreen}RECEDING  }\rm'  'Variance' sprintf(' \\bf%.3f\\rm',variance*1e7) ' Location: ' sprintf(' \\bf%.0f\\rm',range_loc_torso) newline ...
                                                    'Walking velocity: '  sprintf('\\bf%.3f\\rm',walking_speed) '\bfm/s\rm, Number of Steps: ' sprintf('\\bf%.0f\\rm',backward_step_counter) ' , Step Length: ' sprintf('\\bf%.3f\\rm',step_length) '\bf m\rm , Cadence: ' sprintf('\\bf%.3f\\rm',cadence) ' \bfsteps/min\rm'];

                        else
                            general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                    'Status: \bf{\color{yellow}RANDOM MOTION  }\rm'  'Variance' sprintf(' \\bf%.3f\\rm',variance*1e7)  ' Location: ' sprintf(' \\bf%.0f\\rm',range_loc_torso) newline];
