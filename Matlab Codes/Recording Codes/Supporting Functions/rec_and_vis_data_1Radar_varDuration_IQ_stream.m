function [fh_baseband]=rec_and_vis_data_1Radar_varDuration_IQ_stream(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
        ...
        if visualise_data
            ...
            if info_1>round(history_duration/dt,0)
                ...
                if reset_steps
                    ...
                    min_dist = 150;
                    step_length = 0;
                    peak_loc = zeros(1,3); % Store peak locations and value to compare them.
                end
                            ...
                            if sign(walking_speed)==1
                                ...
                                if possible_peak && (past_vals(2)<past_vals(1))
                                    ...
                                    if ((info_1 - peak_loc(1))<=min_dist) && (any(peak_loc))
                                        if mean(maxk(upper_envelope,3))>peak_loc(2)
                                            peak_loc = [past_vals_locs(2),info_1,mean(maxk(upper_envelope,3))];
%                                             peak_loc = [info_1,mean(maxk(upper_envelope,3))];
                                        end
                                    elseif ((info_1 - peak_loc(1))>min_dist) && (any(peak_loc))
                                        step_length = abs(range_vector(past_vals_locs(2)) - range_vector(peak_loc(1)));
...
%                                         peak_loc = [info_1,mean(maxk(upper_envelope,3))];
                                    ...
                                    if ~any(peak_loc)
                                        peak_loc = [past_vals_locs(2),info_1,mean(maxk(upper_envelope,3))]; % Update peak_loc values
%                                         peak_loc = [info_1,mean(maxk(upper_envelope,3))]; % Update peak_loc values
                                        forward_step_counter=forward_step_counter+1;
                                    end
                                    possible_peak=0;
                                end
                            ...
                            elseif sign(walking_speed)==-1
                                past_vals = circshift(past_vals,-1);
                                past_vals(2) = mean(maxk(-(lower_envelope),3)); % Current value
                                if ~possible_peak && (past_vals(2)>past_vals(1))
                                    possible_peak = 1;
                                end
                                if possible_peak && (past_vals(2)<past_vals(1))
                                    local_peak_counter = local_peak_counter+1;
                                    if ((info_1 - peak_loc(1))<=min_dist) && (any(peak_loc))
                                        if mean(maxk(-(lower_envelope),3))>peak_loc(2)
                                            peak_loc = [info_1,mean(maxk(-(lower_envelope),3))];
                                        end
                                    elseif ((info_1 - peak_loc(1))>min_dist) && (any(peak_loc))
...
%                 ph_doppler.CData = 20*log10(abs(conj(STFT))+eps);
                ...
                if sign(walking_speed)==1
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') ...
                                                    newline 'Direction: APPROACHING , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s' ...
                                                    newline 'Estimated Number of Steps: ' num2str(forward_step_counter) ' , Estimated Step Length: ' num2str(step_length) ' m']);
                elseif sign(walking_speed)==-1
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline 'Direction: RECEDING    , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s' newline 'Estimated Number of Steps: ' num2str(backward_step_counter)]);
                else
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline 'Direction: NO MOTION   , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s' newline 'Estimated Number of Steps: 0' ]);
                end
                drawnow;
            end
        else
