function [fh_baseband]=rec_and_vis_data_1Radar_varDuration_IQ_stream_working_7(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
...
%                 ph_doppler.CData = 20*log10(abs(conj(STFT))+eps);
                ...
                if sign(walking_speed)==1
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                        ...
                                                        'Estimated Number of Forward Steps: ' num2str(forward_step_counter) ' , Estimated Step Length: ' num2str(step_length) ' m , Estimated Cadence: ' num2str(cadence) ' steps/min']);
                elseif sign(walking_speed)==-1
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                    'Direction: RECEDING    , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s' newline ...
                                                    'Estimated Number of Backward Steps: ' num2str(backward_step_counter) ' , Estimated Step Length: ' num2str(step_length) ' m , Estimated Cadence: ' num2str(cadence) ' steps/min']);
                else
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                                    'Direction: NO MOTION   , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s' newline ...
                                                    'Estimated Number of Backward Steps: 0  , Estimated Step Length: 0 m , Estimated Cadence: 0 steps/min']);
                end
                drawnow;
            end
        else
