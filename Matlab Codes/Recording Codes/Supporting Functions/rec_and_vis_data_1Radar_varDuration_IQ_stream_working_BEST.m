function [fh_baseband]=rec_and_vis_data_1Radar_varDuration_IQ_stream_working_BEST(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
...
%                     abs_bin = sum(abs(Data_Matrix_1_abs(:,subject_range)),2);
%                     
...
%                     [fftmax,~,FFT_signal,FFTfreq] = fftmax_with_boundarise(temp,fps,breathing_frange);
%                    
%                     
%                     
%                     plot_end = datetime;
...
%                     ph_respiratory.YData = normalize(temp(:),'range');
%                     xlim(ax_respiratory, [frame_vector(1) frame_vector(end)]);
%                     str = ['Estimated Respiratory is:' num2str(round(fftmax*60)),'/min'];
%                     th_respiratory.String = {'Respiratory data';str};
%                     
...
%                 ph_doppler.CData = 20*log10(abs(conj(STFT))+eps);
                ...
                elseif sign(walking_speed)==-1
                    ...
                    general_title.String = ['Recording Radar Data - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline ...
                                            'Direction: \bf{\color{red}NONE        }\rm, Walking velocity: ' sprintf(' \\bf%.3f\\rm',walking_speed) '\bf m/s\rm' newline ...
