function [fh_baseband]=rec_and_vis_data_1Radar_varDuration_IQ_stream_working(xep_1,frame_start,frame_stop,actual_fps,fps,visualise_data,visualise_fps,calibration,freqRes_STFT)
                ...
                if Torso_val>0.95
                    ...
                    if (sign(walking_speed)+sign(Torso_speed))==0 % Opposite to each other
                        ...
                        if abs(Torso_speed)>0.1 %threshold for velocity
                            ...
                            if counter_check<=length(check)
                                check(counter_check) = mean(maxk(upper_envelope,3));
                            ...
                            if sign(walking_speed)==1
                                ...
                                if possible_peak && (past_vals(2)<past_vals(1))
                                    ...
                                    elseif ((info_1 - peak_loc(1))>min_dist) && (any(peak_loc))
                                        ...
                                        step_start(counter_check-2)=0.5;
                                        peak_loc = [info_1,past_vals(1)];
                                    end
                            ...
                            elseif sign(walking_speed)==-1
                                ...
                                if possible_peak && (past_vals(2)<past_vals(1))
                                    local_peak_counter = local_peak_counter+1;
                                    if ((info_1 - peak_loc(1))<=min_dist) && (any(peak_loc))
                                        if mean(maxk(abs(lower_envelope),3))>peak_loc(2)
                                            peak_loc = [info_1,mean(maxk(abs(lower_envelope),3))];
                                        end
                                    elseif ((info_1 - peak_loc(1))>min_dist) && (any(peak_loc))
...
%                 ph_doppler.CData = 20*log10(abs(conj(STFT))+eps);
                ...
                if sign(walking_speed)==1
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline newline 'Direction: APPROACHING , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s']);
                elseif sign(walking_speed)==-1
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline newline 'Direction: RECEDING    , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s']);
                else
                    general_title.String = sprintf(['{\\bfRecording Radar Data} - Date: ' datestr(now, 'dd/mm/yyyy'),' , Time: ',datestr(now,'HH:MM:SS:FFF') newline newline 'Direction: NO MOTION   , Estimated walking velocity: ' sprintf('%.3f',walking_speed) ' m/s']);
                end
                drawnow;
            end
        else
...
figure;plot(check);hold on; stem(step_start*max(check));
